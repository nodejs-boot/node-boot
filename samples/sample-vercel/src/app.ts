...
// (`fs.readdirSync`), but Vercel's Node.js Serverless Functions only ship the subset of files
// statically traced from the function's entry point (via `@vercel/nft`) - untraced directories
// scanned dynamically at runtime are not guaranteed to be present in the deployed bundle. Explicit
// imports here make the whole dependency graph statically discoverable, so it works reliably both
// locally and once deployed.
 ...
 * port. Instead, it is bootstrapped once per Serverless Function instance (cold start) and its
 * `VercelServer` exposes a handler function that Vercel invokes for every incoming request.
 * See `api/[...path].ts`.
 */
@EnableDI(Container)
...
export class VercelSampleApp implements NodeBootApp {
    start(): Promise<NodeBootAppView> {
        return NodeBoot.run(VercelServer, appConfig);
    }
}
