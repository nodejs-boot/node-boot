...
/** Return full server path of given route. */
export function getFullServerPath(route: IRoute): string {
...
export function getFullPath(route: IRoute): string {
    return serverToOpenAPIPath(getFullServerPath(route));
}

/**
 * Return OpenAPI Operation object for given route.
 */
export function getOperation(route: IRoute, schemas: {[p: string]: oa.SchemaObject}): oa.OperationObject {
    ...
    return applyOpenAPIMetadata(cleanedOperation, route);
}
 ...
 */
export function getPaths(routes: IRoute[], schemas: {[p: string]: oa.SchemaObject}): oa.PathObject {
 ...
 */
export function getQueryParams(route: IRoute, schemas: {[p: string]: oa.SchemaObject}): oa.ParameterObject[] {
...
export function getRequestBody(route: IRoute): oa.RequestBodyObject | void {
        ...
        bodyParamMetas.length > 0
            ? bodyParamMetas.reduce(
                  (acc: oa.SchemaObject, d) => ({
                      ...
                      required: isRequired(d, route) ? [...(acc.required || []), d.name!] : acc.required,
                  }),
    ...
    if (bodyMeta) {
        ...
        return {
            content: {
                "application/json": {
                    schema: bodyParamsSchema ? {allOf: [bodySchema, bodyParamsSchema]} : bodySchema,
                },
            },
 ...
 */
export function serverToOpenAPIPath(expressPath: string) {
 ...
 * reflection.
 */
function getParamSchema(param: ParamMetadataArgs): oa.SchemaObject | oa.ReferenceObject {
    const {explicitType, index, object, method} = param;

    const type: (() => unknown) | unknown = Reflect.getMetadata("design:paramtypes", object, method)[index];

    if (typeof type === "function" && type.name === "Array") {
        const items = explicitType ? {$ref: "#/components/schemas/" + explicitType.name} : {type: "object" as const};
