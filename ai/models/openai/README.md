...
OpenAI model adapters for Node-Boot AI, covering all 5 Spring-AI-style model categories:

-   **Chat** — `OpenAiChatModel` (`ChatModel`)
-   **Embedding** — `OpenAiEmbeddingModel` (`EmbeddingModel`)
-   **Image** — `OpenAiImageModel` (`ImageModel`, DALL·E)
-   **Audio** — `OpenAiAudioTranscriptionModel` (`AudioTranscriptionModel`, Whisper) and `OpenAiSpeechModel`
    (`SpeechModel`, TTS)
-   **Moderation** — `OpenAiModerationModel` (`ModerationModel`)

## Usage

```typescript
import {EnableAi} from "@nodeboot/ai-core";
import {EnableOpenAiModels} from "@nodeboot/ai-openai";
...
Each model is registered in the DI container by its concrete class (e.g. `OpenAiChatModel`, `OpenAiEmbeddingModel`,
`OpenAiImageModel`, `OpenAiAudioTranscriptionModel`, `OpenAiSpeechModel`, `OpenAiModerationModel`), so you can inject
it directly with a typed constructor parameter — see the "Injecting the Registered Models" section below.
...
Enabling this package registers every supported model in the DI container by its concrete class (`OpenAiChatModel`, `OpenAiEmbeddingModel`, `OpenAiImageModel`, `OpenAiAudioTranscriptionModel`, `OpenAiSpeechModel`, `OpenAiModerationModel`), plus a default `ChatClient` (as `ChatClient`). Inject them into any `@Service`/`@Controller` simply as a constructor parameter typed with the class (Node-Boot resolves it via reflection, no decorator needed), or with `@Inject()`:
...
export class MyAiService {
    constructor(
        ...
        private readonly openAiEmbeddingModel: OpenAiEmbeddingModel,
        private readonly openAiImageModel: OpenAiImageModel,
        private readonly openAiAudioTranscriptionModel: OpenAiAudioTranscriptionModel,
        private readonly openAiSpeechModel: OpenAiSpeechModel,
        private readonly openAiModerationModel: OpenAiModerationModel,
...
`OpenAiChatModel` — it supports fluent prompt building, `{param}` templating, structured output parsing, default
...
-   **As an MCP server**: `@EnableMcp()` with `mcp.server.enabled: true` in `app-config.yaml` exposes every
    `@Tool` in your app (including tools that call this package's `OpenAiChatModel`-backed `ChatClient`) to external
