...
MiniMax Chat model adapter for Node-Boot AI (OpenAI-compatible API).

MiniMax exposes an OpenAI-compatible Chat Completions API, so this package reuses `OpenAiChatModel`
from `@nodeboot/ai-openai` under the hood, pre-configured with MiniMax's default base URL (`https://api.minimax.io/v1`)
and a sensible default model (`MiniMax-Text-01`).

## Usage

```typescript
import {EnableAi} from "@nodeboot/ai-core";
import {EnableMiniMaxModels} from "@nodeboot/ai-minimax";
...
ai:
    minimax:
        apiKey: "..."
        # baseURL: "https://api.minimax.io/v1" # optional override
        chat:
            options:
...
Enabling this package registers every supported model in the DI container by its concrete class (`MiniMaxChatModel`), plus a default `ChatClient` (as `ChatClient`). Inject them into any `@Service`/`@Controller` simply as a constructor parameter typed with the class (Node-Boot resolves it via reflection, no decorator needed), or with `@Inject()`:

```typescript
import {Inject, Service} from "@nodeboot/core";
import {ChatClient} from "@nodeboot/ai-core";
import {MiniMaxChatModel} from "@nodeboot/ai-minimax";
...
export class MyAiService {
    constructor(
        ...
        private readonly chatModel: MiniMaxChatModel,
...
`MiniMaxChatModel` — it supports fluent prompt building, `{param}` templating, structured output parsing, default
...
-   **As an MCP server**: `@EnableMcp()` with `mcp.server.enabled: true` in `app-config.yaml` exposes every
    `@Tool` in your app (including tools that call this package's `MiniMaxChatModel`-backed `ChatClient`) to external
