...
Mistral AI model adapters for Node-Boot AI, covering 3 Spring-AI-style model categories:

-   **Chat** — `MistralChatModel` (`ChatModel`)
-   **Embedding** — `MistralEmbeddingModel` (`EmbeddingModel`)
-   **Moderation** — `MistralModerationModel` (`ModerationModel`)

## Usage

```typescript
import {EnableAi} from "@nodeboot/ai-core";
import {EnableMistralModels} from "@nodeboot/ai-mistral";
...
Each model is registered in the DI container by its concrete class (e.g. `MistralChatModel`,
`MistralEmbeddingModel`, `MistralModerationModel`), so you can inject it directly with a typed constructor
...
Enabling this package registers every supported model in the DI container by its concrete class (`MistralChatModel`, `MistralEmbeddingModel`, `MistralModerationModel`), plus a default `ChatClient` (as `ChatClient`). Inject them into any `@Service`/`@Controller` simply as a constructor parameter typed with the class (Node-Boot resolves it via reflection, no decorator needed), or with `@Inject()`:

```typescript
import {Inject, Service} from "@nodeboot/core";
import {ChatClient} from "@nodeboot/ai-core";
import {MistralChatModel, MistralEmbeddingModel, MistralModerationModel} from "@nodeboot/ai-mistral";
...
export class MyAiService {
    constructor(
        ...
        private readonly chatModel: MistralChatModel,
        private readonly mistralEmbeddingModel: MistralEmbeddingModel,
        private readonly mistralModerationModel: MistralModerationModel,
...
`MistralChatModel` — it supports fluent prompt building, `{param}` templating, structured output parsing, default
...
-   **As an MCP server**: `@EnableMcp()` with `mcp.server.enabled: true` in `app-config.yaml` exposes every
    `@Tool` in your app (including tools that call this package's `MistralChatModel`-backed `ChatClient`) to external
