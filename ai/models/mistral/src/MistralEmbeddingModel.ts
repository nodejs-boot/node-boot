...
export interface MistralEmbeddingClientLike {
    embeddings: {
...
export class MistralEmbeddingModel implements EmbeddingModel {
    ...
    constructor(client: MistralEmbeddingClientLike, options?: EmbeddingOptions) {
        this.client = client;
        this.options = {
            model: "mistral-embed",
            ...options,
    ...
    async embed(texts: string[] | string): Promise<number[][]> {
        ...
        const res = await this.client.embeddings.create({
            model: this.options.model ?? "mistral-embed",
            inputs: input,
        });
