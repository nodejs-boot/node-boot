...
NVIDIA NIM Chat model adapter for Node-Boot AI (OpenAI-compatible API).

Nvidia exposes an OpenAI-compatible Chat Completions API, so this package reuses `OpenAiChatModel`
from `@nodeboot/ai-openai` under the hood, pre-configured with Nvidia's default base URL (`https://integrate.api.nvidia.com/v1`)
and a sensible default model (`meta/llama-3.1-8b-instruct`).

## Usage

```typescript
import {EnableAi} from "@nodeboot/ai-core";
import {EnableNvidiaModels} from "@nodeboot/ai-nvidia";
...
ai:
    nvidia:
        apiKey: "..."
        # baseURL: "https://integrate.api.nvidia.com/v1" # optional override
        chat:
            options:
...
export class MyAiService {
    constructor(
        ...
        private readonly chatModel: NvidiaChatModel,
...
`NvidiaChatModel` — it supports fluent prompt building, `{param}` templating, structured output parsing, default
...
-   **As an MCP server**: `@EnableMcp()` with `mcp.server.enabled: true` in `app-config.yaml` exposes every
    `@Tool` in your app (including tools that call this package's `NvidiaChatModel`-backed `ChatClient`) to external
