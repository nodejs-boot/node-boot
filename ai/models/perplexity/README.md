# @nodeboot/ai-perplexity

Perplexity Chat model adapter for Node-Boot AI (OpenAI-compatible API).

Perplexity exposes an OpenAI-compatible Chat Completions API, so this package reuses `OpenAiChatModel`
from `@nodeboot/ai-openai` under the hood, pre-configured with Perplexity's default base URL (`https://api.perplexity.ai`)
and a sensible default model (`sonar`).
...
ai:
    perplexity:
        apiKey: "..."
        # baseURL: "https://api.perplexity.ai" # optional override
        chat:
            options:
                model: "sonar"
                temperature: 0.7
...
Enabling this package registers every supported model in the DI container by its concrete class (`PerplexityChatModel`), plus a default `ChatClient` (as `ChatClient`). Inject them into any `@Service`/`@Controller` simply as a constructor parameter typed with the class (Node-Boot resolves it via reflection, no decorator needed), or with `@Inject()`:

```typescript
import {Inject, Service} from "@nodeboot/core";
import {ChatClient} from "@nodeboot/ai-core";
import {PerplexityChatModel} from "@nodeboot/ai-perplexity";
...
export class MyAiService {
    constructor(
        ...
        private readonly chatModel: PerplexityChatModel,
...
`PerplexityChatModel` — it supports fluent prompt building, `{param}` templating, structured output parsing, default
...
-   **As an MCP server**: `@EnableMcp()` with `mcp.server.enabled: true` in `app-config.yaml` exposes every
    `@Tool` in your app (including tools that call this package's `PerplexityChatModel`-backed `ChatClient`) to external
