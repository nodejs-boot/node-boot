...
Groq Chat model adapter for Node-Boot AI (OpenAI-compatible API).

Groq exposes an OpenAI-compatible Chat Completions API, so this package reuses `OpenAiChatModel`
from `@nodeboot/ai-openai` under the hood, pre-configured with Groq's default base URL (`https://api.groq.com/openai/v1`)
and a sensible default model (`llama-3.3-70b-versatile`).

## Usage

```typescript
import {EnableAi} from "@nodeboot/ai-core";
import {EnableGroqModels} from "@nodeboot/ai-groq";
...
Enabling this package registers every supported model in the DI container by its concrete class (`GroqChatModel`), plus a default `ChatClient` (as `ChatClient`). Inject them into any `@Service`/`@Controller` simply as a constructor parameter typed with the class (Node-Boot resolves it via reflection, no decorator needed), or with `@Inject()`:

```typescript
import {Inject, Service} from "@nodeboot/core";
import {ChatClient} from "@nodeboot/ai-core";
import {GroqChatModel} from "@nodeboot/ai-groq";
...
export class MyAiService {
    constructor(
        ...
        private readonly chatModel: GroqChatModel,
...
`GroqChatModel` — it supports fluent prompt building, `{param}` templating, structured output parsing, default
...
-   **As an MCP server**: `@EnableMcp()` with `mcp.server.enabled: true` in `app-config.yaml` exposes every
    `@Tool` in your app (including tools that call this package's `GroqChatModel`-backed `ChatClient`) to external
