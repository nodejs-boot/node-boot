...
Docker Model Runner Chat model adapter for Node-Boot AI (OpenAI-compatible local API).

Dmr exposes an OpenAI-compatible Chat Completions API, so this package reuses `OpenAiChatModel`
from `@nodeboot/ai-openai` under the hood, pre-configured with Dmr's default base URL (`http://localhost:12434/engines/llama.cpp/v1`)
and a sensible default model (`ai/smollm2`).

## Usage

```typescript
import {EnableAi} from "@nodeboot/ai-core";
import {EnableDmrModels} from "@nodeboot/ai-dmr";
...
ai:
    dmr:
        apiKey: "..."
        # baseURL: "http://localhost:12434/engines/llama.cpp/v1" # optional override
        chat:
            options:
                model: "ai/smollm2"
                temperature: 0.7
...
export class MyAiService {
    constructor(
        ...
        private readonly chatModel: DmrChatModel,
...
`DmrChatModel` — it supports fluent prompt building, `{param}` templating, structured output parsing, default
...
-   **As an MCP server**: `@EnableMcp()` with `mcp.server.enabled: true` in `app-config.yaml` exposes every
    `@Tool` in your app (including tools that call this package's `DmrChatModel`-backed `ChatClient`) to external
