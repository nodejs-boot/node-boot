/**
 * Integration tests for the `@nodeboot/fastify-server` driver.
 *
 * Boots a real Node-Boot application on `FastifyServer` (via `@nodeboot/node-test`'s `useNodeBoot`)
...
describe("Fastify Server Adapter", () => {
    ...
    const {useHttp} = useNodeBoot(TestApp as any, ({useConfig}) => {
        useConfig({app: {name: "fastify-server-test", port: TEST_PORT}});
    });
