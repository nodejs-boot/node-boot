/**
 * Integration tests for the `@nodeboot/http-server` driver (native Node.js `http` + `find-my-way`).
 *
 * Boots a real Node-Boot application on `HttpServer` (via `@nodeboot/node-test`'s `useNodeBoot`)
...
const TEST_PORT = 34595;

describe("Native HTTP Server Adapter", () => {
    ...
    const {useHttp} = useNodeBoot(TestApp as any, ({useConfig}) => {
        useConfig({app: {name: "http-server-test", port: TEST_PORT}});
    });
