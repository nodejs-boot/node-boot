/**
 * Integration tests for the `@nodeboot/encore-server` driver.
 *
 * `EncoreServer` doesn't own an HTTP listener itself (Encore.ts's own runtime calls
 * `getHandler()` as a raw endpoint) - `fixtures/TestApp.ts` wraps that same handler in a real
 * `http.Server` so `useHttp()` behaves exactly as it would for any other adapter. Covers the full
 * request/response surface: routing, path/query params, headers, cookies, body parsing +
...
import {TestApp, closeTestServer} from "./fixtures/TestApp";
import {requestLog} from "./fixtures/RequestLogMiddleware";

const TEST_PORT = 34594;

describe("Encore Server Adapter", () => {
    ...
    const {useHttp} = useNodeBoot(TestApp as any, ({useConfig}) => {
        useConfig({app: {name: "encore-server-test", port: TEST_PORT}});
    });
    ...
    describe("routing", () => {
        test("a route declared with a trailing slash is also reachable without one", async () => {
            // find-my-way is registered with `ignoreTrailingSlash: true` for this adapter.
