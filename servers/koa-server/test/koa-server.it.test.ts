/**
 * Integration tests for the `@nodeboot/koa-server` driver.
 *
 * Boots a real Node-Boot application on `KoaServer` (via `@nodeboot/node-test`'s `useNodeBoot`)
...
describe("Koa Server Adapter", () => {
    ...
    const {useHttp} = useNodeBoot(TestApp as any, ({useConfig}) => {
        useConfig({app: {name: "koa-server-test", port: TEST_PORT}});
    });
