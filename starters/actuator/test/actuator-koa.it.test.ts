/**
 * Auto-configuration integration test for `@nodeboot/starter-actuator` with Koa server adapter.
 */
import {before, describe, test} from "node:test";
import assert from "node:assert/strict";
import {hostname} from "node:os";
import {useNodeBoot} from "@nodeboot/node-test";
import {ActuatorKoaApp} from "./fixtures/ActuatorKoaApp";

const TEST_PORT = 38104;

describe("@nodeboot/starter-actuator - Koa server adapter", () => {
    const {useHttp} = useNodeBoot(ActuatorKoaApp as any, ({useConfig}) => {
        useConfig({
            app: {name: "starter-actuator-koa-test", port: TEST_PORT},
            custom: {framework: "koa"},
        });
    });
    ...
    test("exposes /actuator/config with application config", async () => {
        ...
        assert.equal(response.data.app?.name, "starter-actuator-koa-test");
        assert.equal(response.data.custom?.framework, "koa");
    });
