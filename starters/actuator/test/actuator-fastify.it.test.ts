/**
 * Auto-configuration integration test for `@nodeboot/starter-actuator` with Fastify server adapter.
 */
import {before, describe, test} from "node:test";
import assert from "node:assert/strict";
import {hostname} from "node:os";
import {useNodeBoot} from "@nodeboot/node-test";
import {ActuatorFastifyApp} from "./fixtures/ActuatorFastifyApp";

const TEST_PORT = 38103;

describe("@nodeboot/starter-actuator - Fastify server adapter", () => {
    const {useHttp} = useNodeBoot(ActuatorFastifyApp as any, ({useConfig}) => {
        useConfig({
            app: {name: "starter-actuator-fastify-test", port: TEST_PORT},
            custom: {envName: "test-fastify"},
        });
    });
    ...
    test("exposes /actuator/config with application config", async () => {
        ...
        assert.equal(response.data.app?.name, "starter-actuator-fastify-test");
        assert.equal(response.data.custom?.envName, "test-fastify");
    });
