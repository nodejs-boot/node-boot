/**
 * Auto-configuration integration test for `@nodeboot/starter-actuator` with Hono server adapter.
 */
import {before, describe, test} from "node:test";
import assert from "node:assert/strict";
import {hostname} from "node:os";
import {useNodeBoot} from "@nodeboot/node-test";
import {ActuatorHonoApp} from "./fixtures/ActuatorHonoApp";

const TEST_PORT = 38105;

describe("@nodeboot/starter-actuator - Hono server adapter", () => {
    const {useHttp} = useNodeBoot(ActuatorHonoApp as any, ({useConfig}) => {
        useConfig({
            app: {name: "starter-actuator-hono-test", port: TEST_PORT},
            custom: {runtime: "hono"},
        });
    });
    ...
    test("exposes /actuator/config with application config", async () => {
        ...
        assert.equal(response.data.app?.name, "starter-actuator-hono-test");
        assert.equal(response.data.custom?.runtime, "hono");
    });
