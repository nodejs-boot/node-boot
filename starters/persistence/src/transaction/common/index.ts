...
const patchDataSource = (dataSource: DataSource) => {
    ...
    Object.defineProperty(dataSource, "manager", {
        configurable: true,
        get() {
            return getEntityManagerInContext(this[TYPEORM_DATA_SOURCE_NAME] as DataSourceName) || originalManager;
        },
    ...
    dataSource.query = function <T = any>(...args: unknown[]): Promise<T> {
        args[2] = args[2] || this.manager?.queryRunner;

        return originalQuery.apply(this, args as any) as Promise<T>;
    };
    ...
    dataSource.createQueryBuilder = function (...args: unknown[]) {
        ...
        return originalCreateQueryBuilder.apply(this, args as any);
    };
...
export const initializeTransactionalContext = (options?: Partial<TypeormTransactionalOptions>) => {
    ...
    const getRepository = (originalFn: (...args: any[]) => any) => {
        return function patchRepository(this: any, ...args: any[]) {
            const repository = originalFn.apply(this, args) as Record<string, any>;
    ...
    EntityManager.prototype.getRepository = getRepository(
        originalGetRepository,
    ) as typeof EntityManager.prototype.getRepository;
    Repository.prototype.extend = getRepository(originalExtend) as typeof Repository.prototype.extend;
