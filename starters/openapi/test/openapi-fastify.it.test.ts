/**
 * Integration test for `@nodeboot/starter-openapi` with Fastify server adapter.
 */
import {before, describe, test} from "node:test";
import assert from "node:assert/strict";
import {useNodeBoot} from "@nodeboot/node-test";
import {OpenApiFastifyApp} from "./fixtures/OpenApiFastifyApp";

const TEST_PORT = 38203;

describe("@nodeboot/starter-openapi - Fastify server adapter", () => {
    const {useHttp} = useNodeBoot(OpenApiFastifyApp as any, ({useConfig}) => {
        useConfig({
            app: {name: "starter-openapi-fastify-test", port: TEST_PORT},
            openapi: {
                info: {
                    title: "Node-Boot Starter OpenAPI Fastify Test",
                    version: "2.6.0",
                    description: "OpenAPI specification on Fastify",
    ...
    test("serves OpenAPI 3.0 spec at /api-docs/swagger.json with all configured metadata", async () => {
        ...
        assert.equal(response.data.info.title, "Node-Boot Starter OpenAPI Fastify Test");
        assert.equal(response.data.info.version, "2.6.0");
        assert.equal(response.data.info.description, "OpenAPI specification on Fastify");
