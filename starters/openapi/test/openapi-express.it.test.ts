/**
 * Integration test for `@nodeboot/starter-openapi` with Express server adapter.
 */
import {before, describe, test} from "node:test";
import assert from "node:assert/strict";
import {useNodeBoot} from "@nodeboot/node-test";
import {OpenApiExpressApp} from "./fixtures/OpenApiExpressApp";

const TEST_PORT = 38202;

describe("@nodeboot/starter-openapi - Express server adapter", () => {
    const {useHttp} = useNodeBoot(OpenApiExpressApp as any, ({useConfig}) => {
        useConfig({
            app: {name: "starter-openapi-express-test", port: TEST_PORT},
            openapi: {
                info: {
                    title: "Node-Boot Starter OpenAPI Express Test",
                    version: "2.6.0",
                    description: "OpenAPI specification on Express",
    ...
    test("serves OpenAPI 3.0 spec at /api-docs/swagger.json with all configured metadata", async () => {
        ...
        assert.equal(response.data.info.title, "Node-Boot Starter OpenAPI Express Test");
        assert.equal(response.data.info.version, "2.6.0");
        assert.equal(response.data.info.description, "OpenAPI specification on Express");
