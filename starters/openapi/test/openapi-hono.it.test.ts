/**
 * Integration test for `@nodeboot/starter-openapi` with Hono server adapter.
 */
import {before, describe, test} from "node:test";
import assert from "node:assert/strict";
import {useNodeBoot} from "@nodeboot/node-test";
import {OpenApiHonoApp} from "./fixtures/OpenApiHonoApp";

const TEST_PORT = 38205;

describe("@nodeboot/starter-openapi - Hono server adapter", () => {
    const {useHttp} = useNodeBoot(OpenApiHonoApp as any, ({useConfig}) => {
        useConfig({
            app: {name: "starter-openapi-hono-test", port: TEST_PORT},
            openapi: {
                info: {
                    title: "Node-Boot Starter OpenAPI Hono Test",
                    version: "2.6.0",
                    description: "OpenAPI specification on Hono",
    ...
    test("serves OpenAPI 3.0 spec at /api-docs/swagger.json with all configured metadata", async () => {
        ...
        assert.equal(response.data.info.title, "Node-Boot Starter OpenAPI Hono Test");
        assert.equal(response.data.info.version, "2.6.0");
        assert.equal(response.data.info.description, "OpenAPI specification on Hono");
