/**
 * Integration test for `@nodeboot/starter-openapi` with Native HTTP server adapter.
 */
import {before, describe, test} from "node:test";
import assert from "node:assert/strict";
import {useNodeBoot} from "@nodeboot/node-test";
import {OpenApiHttpApp} from "./fixtures/OpenApiHttpApp";

const TEST_PORT = 38201;

describe("@nodeboot/starter-openapi - Native HTTP server adapter", () => {
    const {useHttp} = useNodeBoot(OpenApiHttpApp as any, ({useConfig}) => {
        useConfig({
            app: {name: "starter-openapi-http-test", port: TEST_PORT},
            openapi: {
                info: {
                    title: "Node-Boot Starter OpenAPI Native HTTP Test",
                    version: "2.6.0",
                    description: "OpenAPI specification on Native HTTP",
    ...
    test("serves OpenAPI 3.0 spec at /api-docs/swagger.json with all configured metadata", async () => {
        ...
        assert.equal(response.data.info.title, "Node-Boot Starter OpenAPI Native HTTP Test");
        assert.equal(response.data.info.version, "2.6.0");
        assert.equal(response.data.info.description, "OpenAPI specification on Native HTTP");
